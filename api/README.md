...
- **Reverse Proxy Enforcement**: All requests must originate from a dedicated reverse proxy; direct access is denied. (see `api_keys.upstream_server_protection` in ./config/default.json) 
- **IP Whitelisting**:  
...
docker compose run -it --rm --build api-client
```

#### Modifying code for easy downloads
To make it easy to download files from the browser, we need to add `-J -O` to the curl command. This will instruct curl to save the file with the name provided in the `Content-Disposition` header and to write the output to a file instead of stdout.

These options are not used in the code generated by openapi generator. So we use a bash script to modify the generated client code.

[postProcessClient.sh](./postProcessClient.sh) is a bash script that modifies the client code to add `-J -O` to the curl command. This script is called by [generateClient.sh](./generateClient.sh) after the client code is generated.

##### Steps to modify the client code manually
- Add `-J -O` to the curl command in the client code in `call_downloadCohortFile` function

Add below snippet to [clients/bash/biobank-client](./clients/bash/biobank-client) to add `-J -O` to the curl command. 

```bash
    # Add -J and -O to curl arguments if not already present
    # Manually addition start
    if [[ ! "$curl_arguments" =~ "-J" ]]; then
      curl_arguments+=" -J"
    fi
    if [[ ! "$curl_arguments" =~ "-O" ]]; then
      curl_arguments+=" -O"
    fi
...
#### Deploy bash client to a VM.
1. Copy `clients/bash` directory to a VM. Example path: `/soft/biobank-client`
2. `chmod u+x biobank-client` - make the client executable
3. Add these lines to `.bash_profile`

```bash
source /soft/biobank-client/biobank-client.bash-completion
export "PATH=$PATH:/soft/biobank-client/biobank-client"
if [ -f ~/.env ]; then
  export $(cat ~/.env | xargs)
fi
```

4. Copy `clients/.env.example` to VM user's home directory and rename it to `.env`. Update the file with correct hostname, your API key and secret.