...
function decode_chromosome(encoded) {
  ...
  return mapping[encoded] || encoded.toString();
}
...
function encode_zygosities(decoded) {
  return decoded.map((zygosity) => zygMapping[zygosity]);
}
...
const schema = {
  ...
  required: ['filters', 'zygosities', 'ranges', 'snapshot_id', 'source_id'],
  additionalProperties: false,
  definitions: {
    query: {
      type: 'object',
...
function sanitizeFiltersTree(queryJson) {
...
function sanitize(query_body) {
  return {
    ...query_body,
    filters: sanitizeFiltersTree(query_body.filters),
    ranges: sanitizeRanges(query_body.ranges),
    zygosities: encode_zygosities(query_body.zygosities),
  };
}
...
function rangeToJSON(range) {
  if (range.type === 'region') {
    return {
      ...
      value: {
        ...range.value,
        chr: decode_chromosome(range.value.chr),
      },
    };
  }
  if (range.type === 'variant') {
