...
async function importHospitalData() {
  ...
  await asyncForEach(data, async (row, index) => {
    const { STUDY_ID, IB_ID_LONG, ENC_ID, DEID_ADMIT, DEID_DISCHARGE, DX_CODE, DX_CODE_SYSTEM } = row;
    ...
    if (!STUDY_ID || !IB_ID_LONG) {
      ...
      if (!participant) {
        participant = await prisma.participant.create({ data: { ib_id: IB_ID_LONG, study_id: Number(STUDY_ID) } });
      }
