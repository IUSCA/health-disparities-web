...
router.get('/search/tree', asyncHandler(async (req, res, next) => {
...
router.get('/synonyms', asyncHandler(async (req, res, next) => {
  const { keyword } = req.query;
  if (!keyword) {
    return res.json([]);
  }
...
router.get('/search/tree2', asyncHandler(async (req, res, next) => {
  const { keyword } = req.query;
  if (!keyword) {
    return res.json([]);
  }
